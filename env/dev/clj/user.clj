(ns user
  ...
  (:require
    ...
    [shogidude.pforacle2024.core :refer [start-app]]))
...
(defn dev-prep!
  ...
  (integrant.repl/set-prep! (fn []
                              (-> (shogidude.pforacle2024.config/system-config {:profile :dev})
                                  (ig/prep)))))

(defn test-prep!
  []
  (integrant.repl/set-prep! (fn []
                              (-> (shogidude.pforacle2024.config/system-config {:profile :test})
                                  (ig/prep)))))
